...
func (s *S3SnapshotRestoreTestSuite) SetupSuite() {
	...
	if rancherConfig.ClusterName == "" {
		...
		cluster, err := client.Steve.SteveType(stevetypes.Provisioning).ByID("fleet-default/" + s.client.RancherConfig.ClusterName)
		clusterType, err := clusters.GetClusterType(client, rancherConfig.ClusterName)
		require.NoError(s.T(), err)

		if clusterType == defaults.RKE2 {
			s.rke2Cluster = cluster
		} else if clusterType == defaults.K3S {
			s.k3sCluster = cluster
		}
	}
}
...
func (s *S3SnapshotRestoreTestSuite) TestS3SnapshotRestore() {
	...
	tests := []struct {
		...
		{"RKE2_S3_Restore", snapshotRestoreNone, s.rke2Cluster, defaults.RKE2},
		{"K3S_S3_Restore", snapshotRestoreNone, s.k3sCluster, defaults.K3S},
	}
	...
	for _, tt := range tests {
		...
		s.Run(tt.name, func() {
			...
			if tt.cluster == nil {
				s.T().Skipf("Cluster provided is not %s", tt.clusterType)
			}
